...
function PatientCaregiver({ user }) {
  ...
  return (
    <div>
      ...
      <div style={{ ...S.card, marginBottom: 14 }}>
        <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 14 }}>Linked caregiver</div>
        {linked ? (
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', background: C.sucLt, borderRadius: 12, marginBottom: 14, border: `1px solid #9FE1CB` }}>
              <div style={{ width: 44, height: 44, borderRadius: '50%', background: C.priXl, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.pri, fontWeight: 700, fontSize: 14, flexShrink: 0 }}>MJ</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700, fontSize: 14 }}>Mary Johnson</div>
                <div style={{ color: C.mut, fontSize: 12 }}>mary@gmail.com · Daughter · +61 422 222 222</div>
              </div>
              <span style={{ background: C.sucLt, color: C.suc, padding: '4px 12px', borderRadius: 20, fontSize: 12, fontWeight: 700, border: `1px solid ${C.sucMd}` }}>
                <i className="ti ti-circle-check" style={{ fontSize: 12 }} aria-hidden="true" /> Active
              </span>
            </div>
            <div style={{ fontWeight: 700, fontSize: 12, color: C.mut, letterSpacing: 0.5, marginBottom: 10 }}>WHAT YOUR CAREGIVER CAN SEE</div>
            {["Today's medication schedule","Missed dose alerts in real-time","Weekly adherence reports","Your medication list & dosages","Emergency contact info"].map(item => (
            ...
            <button onClick={() => setLinked(false)} style={{ ...S.btnGhost, marginTop: 14, color: C.danDk, borderColor: C.dan, background: C.danLt }}>
              <i className="ti ti-user-minus" aria-hidden="true" /> Remove caregiver
            </button>
          </>
        ) : (
          <div>
            <p style={{ fontSize: 13, color: C.mut, marginBottom: 14 }}>A caregiver can monitor your medications and receive alerts when you miss a dose.</p>
            <div style={{ marginBottom: 12 }}>
              <label style={{ fontSize: 12, fontWeight: 600, color: C.mut, display: 'block', marginBottom: 5 }}>Caregiver email address</label>
              <input style={{ ...S.input }} placeholder="caregiver@email.com" value={form.patient_email} onChange={e => setForm(p => ({ ...p, patient_email: e.target.value }))} />
            </div>
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 12, fontWeight: 600, color: C.mut, display: 'block', marginBottom: 5 }}>Relationship</label>
              <input style={{ ...S.input }} placeholder="e.g. Daughter, Son, Nurse" value={form.relationship} onChange={e => setForm(p => ({ ...p, relationship: e.target.value }))} />
            </div>
            <button onClick={() => setLinked(true)} style={{ ...S.btnPrimary(C.pri) }}>
              <i className="ti ti-user-plus" aria-hidden="true" /> Link caregiver
            </button>
          </div>
        )}
      </div>
      <div style={{ ...S.card }}>
        <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 14 }}>Recent alerts sent to caregiver</div>
        {[['Vitamin D3 missed at 13:00','Today, 1:30 PM','missed'],['Metformin taken at 08:05','Today, 8:05 AM','taken'],['Weekly report delivered','Yesterday, 9:00 AM','info']].map(([msg,dt,t]) => (
          <div key={msg} style={{ display: 'flex', gap: 11, alignItems: 'flex-start', padding: '10px 0', borderBottom: `1px solid #f5f5f5` }}>
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: t==='missed'?C.dan:t==='taken'?C.suc:C.infMd, marginTop: 5, flexShrink: 0 }} />
            <div><div style={{ fontSize: 13 }}>{msg}</div><div style={{ fontSize: 11, color: C.mut }}>{dt}</div></div>
          </div>
        ))}
      </div>
    </div>
