    ...
    <div style={{ minHeight: '100vh', display: 'flex', fontFamily: "-apple-system,'Segoe UI',sans-serif" }}>
      ...
      <div style={{ width: 440, background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 36px' }}>
        <div style={{ width: '100%', maxWidth: 360 }}>
          ...
          <div style={{ marginTop: 24, padding: '14px', background: '#f8f8f8', borderRadius: 10, fontSize: 12, color: C.mut }}>
            <div style={{ fontWeight: 700, marginBottom: 6, color: C.txt }}>Test accounts:</div>
            {[['Admin', 'admin@medicare.com', 'Admin@1234'], ['Patient', 'patient@test.com', 'Test@1234'], ['Caregiver', 'caregiver@test.com', 'Test@1234']].map(([role, email, pass]) => (
              <div key={role} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                <span style={{ fontWeight: 600 }}>{role}:</span>
                <span style={{ cursor: 'pointer', color: C.pri }} onClick={() => setForm(f => ({ ...f, email, password: pass }))}>{email}</span>
              </div>
            ))}
          </div>
        </div>
