  ...
  const go = async () => {
    ...
    try {
      if (mode === 'login') {
        ...
        if (!name) { setError('Enter your full name'); setBusy(false); return; }
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', fontFamily:"Inter,system-ui,sans-serif" }}>
      ...
      <div style={{ flex:1, background:'linear-gradient(160deg,#052e16 0%,#0F6E56 60%,#1D9E75 100%)', display:'flex', flexDirection:'column', justifyContent:'center', padding:'52px 48px', color:'#fff' }}>
        <div style={{ width:54, height:54, background:'#EF9F27', borderRadius:15, display:'flex', alignItems:'center', justifyContent:'center', fontSize:26, marginBottom:22, boxShadow:'0 8px 20px rgba(0,0,0,0.2)' }}>💊</div>
        <h1 style={{ fontSize:34, fontWeight:800, margin:'0 0 10px', fontFamily:'Plus Jakarta Sans,Inter,sans-serif', lineHeight:1.2 }}>MediCare PRO</h1>
        ...
        {[['🔔','Real-time reminders','Push notifications on your devices'],['👥','3 role system','Patient, Caregiver & Admin portals'],['📧','Email alerts','Automatic alerts for missed doses'],['🤖','AI assistant','Medication advice & doctor recommendations']].map(([ic,t,d]) => (
          <div key={t} style={{ display:'flex', gap:13, marginBottom:18, alignItems:'flex-start' }}>
      ...
      <div style={{ width:460, display:'flex', alignItems:'center', justifyContent:'center', padding:40, background:'#FAFAFA' }}>
        <div style={{ width:'100%', maxWidth:370, background:'#fff', borderRadius:16, padding:'32px 28px', boxShadow:'0 4px 24px rgba(0,0,0,0.08)' }}>
          <h2 style={{ fontSize:24, fontWeight:700, margin:'0 0 4px', color:'#111827' }}>
            {mode==='login' ? 'Welcome back' : 'Create account'}
          </h2>
          <p style={{ color:'#6B7280', fontSize:13, margin:'0 0 24px' }}>
            {mode==='login' ? 'Sign in to your MediCare account' : 'Join MediCare to manage your medications'}
          </p>
          ...
          <button onClick={go} disabled={busy} style={{ width:'100%', padding:12, borderRadius:10, border:'none', cursor:busy?'not-allowed':'pointer', background:busy?'#9CA3AF':'linear-gradient(135deg,#052e16,#0F6E56)', color:'#fff', fontSize:14, fontWeight:700, fontFamily:'inherit', boxShadow:'0 4px 14px rgba(15,110,86,0.25)', transition:'all 0.15s' }}>
