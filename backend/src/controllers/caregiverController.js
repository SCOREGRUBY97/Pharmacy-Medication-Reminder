...
const linkCaregiverByEmail = async (req, res) => {
  ...
  if (!caregiver_email) return res.status(400).json({ error: 'Caregiver email required' });
  try {
    ...
    if (!caregiver.rows.length) return res.status(404).json({ error: 'No active caregiver found with that email' });
    if (caregiver.rows[0].id === req.user.id) return res.status(400).json({ error: 'Cannot link yourself' });
    await query(`INSERT INTO caregiver_patients(caregiver_id,patient_id,relationship)
      VALUES($1,$2,$3) ON CONFLICT(caregiver_id,patient_id)
    ...
    await query(`INSERT INTO notifications(user_id,type,channel,title,message)
      ...
      [caregiver.rows[0].id, `${req.user.full_name} linked you as their caregiver.`]).catch(() => {});
    res.json({ message: `Successfully linked caregiver ${caregiver.rows[0].full_name}!`, caregiver: caregiver.rows[0] });
  } catch (err) {
...
const unlinkCaregiver = async (req, res) => {
  try {
    await query('UPDATE caregiver_patients SET is_active=false WHERE patient_id=$1 AND caregiver_id=$2',
...
module.exports = { getPatients, getPatientOverview, linkPatient, unlinkPatient, getAlerts, getCaregiversForPatient, linkCaregiverByEmail, unlinkCaregiver };
