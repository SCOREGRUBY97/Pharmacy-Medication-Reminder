...
const updateStatus = async (req, res) => {
  ...
  try {
    ...
    if (['taken','missed','skipped'].includes(status)) {
      ...
      // When a patient manually marks a dose as missed, notify linked caregivers immediately.
      if (status === 'missed') {
        const caregivers = await query(`
          SELECT u.id, u.full_name FROM caregiver_patients cp
          JOIN users u ON cp.caregiver_id = u.id
          WHERE cp.patient_id=$1 AND cp.is_active=true AND u.is_active=true`, [req.user.id]);
        ...
        for (const caregiver of caregivers.rows) {
          await query(`INSERT INTO notifications(user_id,reminder_id,type,channel,title,message)
            VALUES($1,$2,'caregiver_alert','in_app',$3,$4)`,
            [caregiver.id, rem.id, `Missed dose: ${req.user.full_name}`, `${req.user.full_name} marked ${rem.med_name} as missed.`]);
        }
      }
    }
